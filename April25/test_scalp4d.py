...
# ─────────────────────────────────────────────
# SPREAD — entry only (limit order exits, no spread on exit)
# ─────────────────────────────────────────────
SPREAD_PCT = 0.0001   # 0.01% — BTC bid/ask spread, applied on entry only
#
# HOW IT WORKS:
#   Long  entry: you buy at ask = close * (1 + 0.01%)  → slightly worse fill
#   Short entry: you sell at bid = close * (1 - 0.01%) → slightly worse fill
#   Exit (SL/TP): limit orders → filled at exact SL/TP price, no spread
#   Net cost per trade: 0.01% of entry price (one-way only)
...
def backtest(df, signal_func, name, sl_pct, rr=RR):
    """
    Entry fill = close ± SPREAD_PCT (you pay the spread to get in).
    SL/TP set from actual entry fill price.
    Exit fill = exact SL/TP price (limit order, no spread on exit).
    PnL = risk_usd × (actual_move / sl_distance).
    """
    tp_pct      = sl_pct * rr
    ...
    for i in range(1, len(df)):
        ...
        if position is not None:
            ...
            if hit_tp or hit_sl:
                # Exit at exact SL/TP — limit order, no spread
        ...
        if position is None and sig != 0 and capital > 0:
            ...
            # Entry fill includes spread — you pay ask on longs, receive bid on shorts
            if sig == 1:
                position   = "long"
                entry_fill = price * (1 + SPREAD_PCT)   # buy at ask
                sl_price   = entry_fill * (1 - sl_pct)
                tp_price   = entry_fill * (1 + tp_pct)
            else:
                position   = "short"
                entry_fill = price * (1 - SPREAD_PCT)   # sell at bid
                sl_price   = entry_fill * (1 + sl_pct)
                tp_price   = entry_fill * (1 - tp_pct)
...
print(f"   Spread: {SPREAD_PCT*100:.2f}% on entry only (limit order exits — no spread on SL/TP)")
print("=" * 100)
